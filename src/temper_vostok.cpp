...
float temp_s, hum_s, pressure_s, pressure_g, hum_t, temp_t;
const char *name_client = "Thermometer-vostok";
...
void callback(char *topic, byte *payload, unsigned int length) // Приём сообщений с Брокера Mqtt
{
  ...
  client.publish("vostok_key", key_char, 1);
  if ((String(topic)) == "Thermometer-vostok_reset")
  {
  ...
  if (OTA_Wifi.isReady()) // Поддержание WiFi и OTA  и Пинок :) watchdog
  {
    ...
    if (!client.connected()) // Проверка на подключение к MQTT
    {
      while (!client.connected())
      {
        ...
        {
          client.subscribe("temper-vostok_reset"); // подписались на топик
    ...
    if (time_20s == 2)
    {
     ...
     // publish_send("bmp_vostok", bmp280_raw);
      ...
      else
      {
        reset++;
        mil_o = 999999;
        publish_send("temp_vostok", mil_o);
        if (reset > 5)
        {
          ESP.restart();
        }
